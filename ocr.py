...
def process_image_with_easyocr(image_path):
	...
	log_scan_info("OCR", EASYOCR_ENGINE, file_path, time_string)
	cv2.destroyAllWindows()
	return TextResult(text=result_text, engine_name=EASYOCR_ENGINE)

# Function to perform OCR on a file
def process_image_with_tesseract(file_path):
...
if args.action_a:
	path = args.action_a
	function_ptrs = [process_image_with_easyocr, process_image_with_tesseract]
elif args.action_t:
	path = args.action_t
	function_ptrs = [process_image_with_tesseract]
elif args.action_e:
	path = args.action_e
	function_ptrs = [process_image_with_easyocr]
else:
	print("Invalid entry")
